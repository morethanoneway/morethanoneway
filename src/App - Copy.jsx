              ...
              return (
                <button
                  ...
                  className={`block w-full text-left py-2 px-3 rounded ${
                    isActive ? "bg-white/10 text-blue-200 font-semibold" : "hover:text-blue-200"
                  }`}
                >
...
const CrisisBanner = () => (
  <div className="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-400 p-5 rounded-xl shadow-soft">
    <div className="flex items-start gap-3">
      <span className="text-3xl">Ã°Å¸Â«"š</span>
      <div>
        ...
        <p className="text-xs text-red-600 italic">
          You matter. This feeling is temporary. People want to help. Ã¢ÂÂ¤Ã¯Â¸Â
        </p>
      </div>
    </div>
  </div>
...
const NavBar = ({ currentPage, setCurrentPage, mobileMenuOpen, setMobileMenuOpen }) => {
  ...
  const resourcesItems = [
    ...
    { page: 'blog', label: 'Blog', icon: <Newspaper className="w-4 h-4" /> },  // ADD THIS
            ...
            <div className="mt-4 border-t border-white/25 pt-4">
              ...
              <div className="flex justify-center gap-6 mb-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex flex-col items-center gap-2 text-white/90 hover:text-cyan-300 transition-all hover:scale-105"
                  aria-label="Follow us on TikTok"
                >
                  <div className="bg-white/10 p-3 rounded-xl hover:bg-cyan-600/80 transition-all">
...
const pageToHash = {
  ...
  'need-a-laugh': 'need-a-laugh',
  volunteer: 'volunteer',
  stories: 'stories',
  pivot: 'pivot',
  about: 'about',
  contact: 'contact',
  'job-tools-hub': 'job-tools-hub',
  resources: 'resources',
  tracker: 'tracker',
  crisis: 'crisis',
  blog: 'blog',
...
const hashToPage = Object.fromEntries(
 ...
 // ✅ Effect A: URL(hash) -> state
  useEffect(() => {
    const applyHashToPage = () => {
      const raw = window.location.hash || '';
      const cleaned = raw.replace(/^#\/?/, ''); // "#/about" -> "about", "#/" -> ""

      // ✅ Blog post route: "#/blog/my-slug"
      ...
      // ✅ Normal pages: "#/blog" "#/about" etc.
      const page = hashToPage[cleaned];

      if (page) {
        setCurrentPage(page);
        // if you go to non-blog-post pages, clear the slug
        if (page !== 'blog-post') setSelectedPostSlug('');
      } else {
        setCurrentPage('home');
        setSelectedPostSlug('');
      }
    ...
    return () => window.removeEventListener('hashchange', applyHashToPage);
  }, []);

  // ✅ Effect B: state -> URL(hash)
  useEffect(() => {
    // Blog post URL should be "#/blog/my-slug"
    if (currentPage === 'blog-post') {
      const next = selectedPostSlug ? `#/blog/${selectedPostSlug}` : '#/blog';
    ...
    <div className="min-h-screen bg-[#FAFAF7] text-gray-900">
      ...
      <main className="w-full px-6 lg:px-12 py-10">
        ...
        {currentPage === 'resume-builder' && (
          <ResumeBuilder onBack={() => setCurrentPage('home')} setCurrentPage={setCurrentPage} />
        )}
        {currentPage === 'study-resources' && <StudyResources onBack={() => setCurrentPage('home')} />}
        {currentPage === 'youre-not-alone' && (
          <YoureNotAlone onBack={() => setCurrentPage('home')} setCurrentPage={setCurrentPage} />
        )}
        {currentPage === 'interview-prep' && <InterviewPrep />}
        {currentPage === 'job-alert' && <JobAlertGuide onBack={() => setCurrentPage('home')} setCurrentPage={setCurrentPage} />}
        {currentPage === 'search-guide' && (
          <SearchGuide
        ...
        {currentPage === 'need-a-laugh' && <NeedALaugh onBack={() => setCurrentPage('home')} setCurrentPage={setCurrentPage} />}
        {currentPage === 'volunteer' && <Volunteer onBack={() => setCurrentPage('home')} setCurrentPage={setCurrentPage} />}
        ...
        {currentPage === 'contact' && <Contact onBack={() => setCurrentPage('home')} setCurrentPage={setCurrentPage} />}
        {currentPage === 'job-tools-hub' && <JobToolsHub setCurrentPage={setCurrentPage} />}
          ...
          <div className="text-center mb-6">
            ...
            <div className="flex justify-center gap-6 mb-6">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex flex-col items-center gap-2 text-gray-300 hover:text-cyan-400 transition-all hover:scale-110"
                aria-label="Follow us on TikTok"
              >
                <div className="bg-gray-700 p-3 rounded-xl hover:bg-cyan-600 transition-all">
                  <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" />
                  </svg>
                </div>
